/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_get_var_env.c                                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: user42 <user42@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/21 16:21:19 by user42            #+#    #+#             */
/*   Updated: 2022/02/22 21:52:43 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/minishell.h"

char	**ft_get_var_env(char **envp, char *var_env)
{
	char	**location;
	char str[] = "/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/sbin";

	while (*envp)
	{
		if (ft_strnstr(*envp, var_env, ft_strlen(var_env)))
		{
			while (**envp != '/')
				*envp += 1;
			location = ft_split(*envp, ':');
			return (location);
		}
		envp++;
	}
	location = ft_split(str, ':');
	return (location);
}

char	**ft_get_var_env_2(char **envp, char *var_env)
{
	char	**location;
	char str[] = "/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/sbin";

	while (*envp)
	{
		if (ft_strncmp(*envp, var_env, ft_strlen(var_env)))
		{
			while (**envp != '/')
				*envp += 1;
			printf("SALUT\n");
			location = ft_split(*envp, '=');
	/*	if (location[0] = "/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/sbin")
			ft_free_charr(location);*/
			return (location);
		}
		envp++;
	}
	printf("SALUT\n");
	location = ft_split(str, '=');
	/*if (location[0] = "/bin:/usr/local/bin:/usr/bin:/bin:/usr/local/sbin")
		location[0] = NULL;*/
	return (location);
}